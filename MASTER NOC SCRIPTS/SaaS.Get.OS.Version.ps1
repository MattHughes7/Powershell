...
$RandomTime1 = (Get-Random (1..5))
$RandomTime2 = (Get-Random (1..5))
$RandomTime = ($RandomTime1 + $RandomTime2)

Sleep $RandomTime1

#Mount Report Dri
Net Use W: "\\AFOX01\REPORT" /persistent:no /user:tor01.desire2learn.d2l\afoxcolo Passowrd
...
ForEach ($TestSites in $TestSites) {
   ...
   if ($output -like "*Login*") {
	...
	$OperatingSystemGet = (Get-WmiObject -class Win32_OperatingSystem).Caption
	
	If ($OperatingSystemGet -match "2003"){
		$OperatingSystem = "2K3"}
		ELSE { $OperatingSystem = "2K8" }
	
	$SuccessString1 = ($TestSites + ", " + $HostName + ", " + "PASSED" + ", " + $OperatingSystem)
	$SuccessString = $SuccessString1.ToUpper()
	Add-Content $ReportFile $SuccessString
	  ...
	  #Report Fail
		...
		$FailedString1 = ($TestSites + ", " + $HostName + ", " + "FAILED" + ", " + $OperatingSystem)
	$FailedString = $FailedString1.ToUpper()
...
N:\Scripts\Analyze.Basic.Check.ps1