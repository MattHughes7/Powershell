...
#check if the kerblist (2003) tool exists
if (!(Test-Path -Path "C:\Windows\System32\kerblist.exe"))
{
	#copy the file from a remote server
	Copy-Item -Path ($logonSessionShare + "kerblist.exe") -Destination "C:\Windows\System32\"
}
